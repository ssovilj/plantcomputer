...
public class WebHandler implements HttpHandler {
    ...
    final MqttClientConnection mqttClientConnection;

    public WebHandler(MqttClientConnection mqttClientConnection) {
...
//            this.queueClient.sendMessage(new FoodComputerCommand(FoodComputerActions.BLINKING, 10, Color.RED, Color.WHITE));
            this.returnSuccess(exchange, "RED ALERT message has been sent");
        } else if (path.equals("/all-white")) {
//            this.queueClient.sendMessage(new FoodComputerCommand(FoodComputerActions.ALL_WHITE, 10, Color.WHITE, Color.BLACK));
            this.returnSuccess(exchange, "ALL WHITE message has been sent");
        } else if (path.equals("/all-out")) {
//            this.queueClient.sendMessage(new FoodComputerCommand(FoodComputerActions.ALL_OUT, 10, Color.BLACK, Color.BLACK));
